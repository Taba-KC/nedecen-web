...
import { lookupLearner, claimLearnerAccount, type PersonName } from "@/lib/onboarding";
...
export default function LearnerOnboardPage() {
  ...
  const admissionActive      = focusedField === "admission"      || admissionNumber !== "";
  ...
  async function handleNext(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    if (!schoolCode || !admissionNumber || !initials) {
      setError("Please fill in all fields.");
      return;
    }
    setLoading(true);
    try {
      const person = await lookupLearner({ schoolCode, admissionNumber, initials });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await claimLearnerAccount({ schoolCode, admissionNumber, initials, username, password });
      setSuccess(true);
    } catch (err: unknown) {
      ...
      >
        <div className="w-full max-w-sm text-center">
          ...
          <p className="text-sm mb-8" style={{ color: "var(--color-text-muted)" }}>
            Your learner account has been set up. You can now sign in with your username and password.
          </p>
          <button
            onClick={() => router.push("/")}
        ...
        <div className="flex flex-col justify-between flex-1 p-12">
          ...
          <div>
            ...
            <p className="text-sm leading-relaxed" style={{ color: "rgba(255,255,255,0.5)" }}>
              Your school has already registered you. Claiming your account takes less than a minute — you just need your school code, admission number, and initials.
            </p>
          </div>
            ...
            {step === 1 && (
              <>
                ...
                <form onSubmit={handleNext} className="flex flex-col" style={{ gap: "var(--field-gap)" }}>
                  ...
                  <div className="relative rounded-lg" style={fieldWrapperStyle(focusedField === "admission")}>
                    <input
                      id="admission"
                      type="text"
                      value={admissionNumber}
                      onChange={(e) => setAdmissionNumber(e.target.value)}
                      onFocus={() => setFocusedField("admission")}
                      onBlur={() => setFocusedField(null)}
                      required
                      className="w-full h-full text-sm outline-none bg-transparent"
                      style={inputStyle(admissionActive)}
                    />
                    <label htmlFor="admission" className="absolute pointer-events-none" style={labelStyle(admissionActive, focusedField === "admission")}>
                      Admission number
                    </label>
                  </div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>
                        Learner account found
                      </p>
                    </div>
                  </div>
                )}
                <p className="text-sm mb-8" style={{ color: "var(--color-text-muted)" }}>
                  Choose a username and password. You will use these every time you sign in.
                </p>
